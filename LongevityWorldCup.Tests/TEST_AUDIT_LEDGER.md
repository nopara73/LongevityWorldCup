...
| Browser fixture architecture | Runtime-derived workload A-D bins shared mutable applications across unrelated classes. The first semantic rewrite also misclassified the analytics-emitting Flow Action Dock matrix as read-only, which exhausted its shared rate limiter on CI. | Rewrite around semantic ownership. | Chromium is collection-scoped; Kestrel is class-scoped only for genuinely read-only classes; browser contexts are test-scoped; Guess, statistics, and the Flow Action Dock matrix use per-test Kestrel. | Not claimed |
...
- Final coverage: 73.78% line, 57.86% branch, 82.99% method. No baseline coverage percentage is claimed because the baseline run failed before report generation.
