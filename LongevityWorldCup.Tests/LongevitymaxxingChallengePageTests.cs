...
public sealed class LongevitymaxxingChallengePageTests
{
    ...
    public async Task ChallengeNoteMentions_ExposeAnAccessibleKeyboardParticipantPicker()
    {
        ...
        Assert.DoesNotContain("/stop-mention-emails", source);
    ...
    public void ChallengeSubmitHelper_IgnoresDuplicateBusySubmissions()
    {
        ...
        Assert.Contains("button.setAttribute(\"aria-busy\", \"true\");", helperBody);
        Assert.Contains("button.removeAttribute(\"aria-busy\");", helperBody);
    }
    ...
    public async Task LongevitymaxxingPage_RendersProductCopyAndVersionedAssets()
    {
        ...
        Assert.DoesNotContain("id=\"lmxMetrics\"", html);
        Assert.DoesNotContain("aria-label=\"Challenge snapshot\"", html);
        Assert.DoesNotContain("id=\"lmxMetricPeople\"", html);
    ...
    public async Task LongevitymaxxingScript_KeepsLeaderboardVisibleAndFocusesDueCheckIn()
    {
        ...
        Assert.Contains("data-collapsed-reply", javascript);
        ...
        Assert.Contains("role=\"dialog\"", javascript);
        Assert.Contains("aria-modal=\"true\"", javascript);
        Assert.Contains("function openNotePhotoViewer", javascript);
        Assert.Contains("function navigateNotePhotoViewer", javascript);
        Assert.Contains("function trapNotePhotoViewerFocus", javascript);
        Assert.Contains("function setNotePhotoViewerPageInert", javascript);
        Assert.Contains("window.history.pushState", javascript);
        ...
        Assert.Contains(".lmx-photo-viewer.show", css);
        Assert.Contains(".lmx-photo-viewer-stage img", css);
        Assert.Contains(".lmx-photo-viewer-close", css);
        ...
        Assert.Contains("class=\"lmx-answer-options\" role=\"radiogroup\"", javascript);
        Assert.Contains("data-answer=\"${answer.tone}\"", javascript);
        Assert.Contains("class=\"lmx-answer-input\" type=\"radio\"", javascript);
        Assert.Contains("aria-label=\"${escAttr(answer.label)}\"", javascript);
        Assert.Contains("class=\"lmx-answer-face\" aria-hidden=\"true\"", javascript);
        Assert.Contains("class=\"lmx-answer-face-icon\"", javascript);
        Assert.Contains("answerFaceIconHtml(answer.tone)", javascript);
        Assert.DoesNotContain("<span>${answer.label}</span>", javascript);
        Assert.Contains("width: 2.5rem;", css);
        Assert.Contains("height: 2.5rem;", css);
        Assert.Contains(".lmx-answer-option[data-answer=\"no\"]", css);
        Assert.Contains("--lmx-answer-accent: #dc2626;", css);
        Assert.Contains(".lmx-answer-option[data-answer=\"somewhat\"]", css);
        Assert.Contains("--lmx-answer-accent: #2563eb;", css);
        Assert.Contains(".lmx-answer-option[data-answer=\"yes\"]", css);
        Assert.Contains("--lmx-answer-accent: #16a34a;", css);
        Assert.Contains("--lmx-answer-accent: var(--lmx-danger);", css);
        Assert.Contains("--lmx-answer-accent: #60a5fa;", css);
        Assert.Contains("--lmx-answer-accent: var(--lmx-success);", css);
        ...
        Assert.Contains("function habitPlantHtml", javascript);
        Assert.Contains("PLANT_YES_GROWTH_RATE = 0.025", javascript);
        Assert.Contains("PLANT_NO_RETENTION_RATE = 0.65", javascript);
        Assert.Contains("function applyGardenAnswer", javascript);
        Assert.Contains("function plantFoliageProgress", javascript);
        Assert.Contains("function plantNodeOpacity", javascript);
        Assert.Contains("function isGardenState", javascript);
        Assert.Contains("isGardenState(value.garden)", javascript);
        Assert.Contains("PLANT_LEAF_CAPACITY = 64", javascript);
        Assert.DoesNotContain("Save today's check-in first.", javascript);
        Assert.DoesNotContain("Saved. You can edit this check-in today.", javascript);
        ...
        Assert.Contains("toggle(\"lmxHeroHighlights\", false);", javascript);
        Assert.DoesNotContain("stats-last", javascript);
        Assert.DoesNotContain(".lmx-title-panel.stats-last", css);
        ...
        Assert.DoesNotContain(".lmx-metric", css);
