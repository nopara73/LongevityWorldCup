...
public sealed class AestheticSystemPageTests
{
    ...
    public async Task GuessMyAge_RestoresBoundedRevealChoreographyWithoutHeavyDependencies()
    {
        ...
        Assert.Contains("const GMA_EXACT_JACKPOT_MS = 9000;", html);
        Assert.Contains("const GMA_EXACT_WAVE_COUNT = 18;", html);
        Assert.Contains("const GMA_EXACT_CONFETTI_CAP = 420;", html);
        Assert.Contains("const GMA_EXACT_TIME_ICON_CAP = 160;", html);
        ...
        Assert.Contains("const outcomeDwell = userError === 0 ? 9600 : 5000;", html);
