...
public sealed class ProofUploadPageTests
{
    ...
    public async Task ResultUpload_UsesTaskSpecificPageHeading()
    {
        ...
        Assert.Contains(">Upload proofs and results</h1>", html);
        Assert.DoesNotContain(">Athlete selection</h1>", html);
    }
    ...
    public async Task ResultUpload_UsesOnePrimaryActionForProofMethodChoices()
    {
        ...
        Assert.Contains("id=\"takeProofPhotoButton\" class=\"option-button flow-action flow-action--secondary camera-capture-action\"", html);
        Assert.Contains("id=\"uploadProofButton\" class=\"option-button green flow-action\"", html);
        Assert.Contains("box-shadow: var(--lwc-shadow-sm", html);
        Assert.DoesNotContain("rgba(46, 125, 50", html);

        var javascript = await GetProofHelpersTypeScriptAsync(client);
        Assert.Contains("uploadProofButton.classList.toggle('green', uploadIsRequired);", javascript);
        Assert.Contains("cameraButton.classList.remove('green');", javascript);
        Assert.Contains("cameraButton.classList.add('grey', 'flow-action--secondary');", javascript);
    }
    ...
    public async Task ProofHelper_ClearsFileInputAfterFailedProofProcessing()
    {
        ...
        Assert.Contains("window.customAlert('Proof upload failed. Please try again with an image or PDF file.')", catchBody);
        Assert.Contains(".then(() => focusProofRetryButton(retryButton));", catchBody);
    ...
    public async Task ProofHelper_DisablesProofControlsWhileProcessingSelection()
    {
        ...
        Assert.Contains("const focusProofRetryButton: (retryButton: HTMLButtonElement | null | undefined) => void = retryButton =>", javascript);
        Assert.Contains("await handleProofFiles(proofPicInput.files, proofPicInput, uploadProofButton);", javascript);
        Assert.Contains("await handleProofFiles(cameraInput.files, cameraInput, cameraButton || uploadProofButton);", javascript);
    ...
    public async Task ProofHelper_FallsBackToRawImageWhenClientOptimizationFails()
    {
        ...
        Assert.Contains("const optimizeProofImageOrFallback: (raw: string) => Promise<string> = async raw =>", javascript);
    ...
    public async Task ProofHelper_UsesSafeStorageForChecklistBiomarkerHandoff()
    {
        ...
        var javascript = await GetProofHelpersTypeScriptAsync(client);
        var helperStart = javascript.IndexOf("function getProofSessionItem(key: string): string | null", StringComparison.Ordinal);
        ...
        var checklistEnd = javascript.IndexOf("function getProofFileExtension(file: File | null | undefined): string", checklistStart, StringComparison.Ordinal);
    ...
    public async Task ProofHelper_IgnoresBlankStoredBiomarkerValuesInChecklist()
    {
        ...
        Assert.Contains("function hasFiniteBiomarkerValue(value: unknown): boolean", javascript);
