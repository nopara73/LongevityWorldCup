...
] as const satisfies readonly PhenoBiomarker[];

function parseInput(value: unknown): number {
    return value === "" ? Number.NaN : Number(value);
}

function markerValue(markerValues: readonly number[], index: number): number {
    return markerValues[index] ?? Number.NaN;
}

function currentPhenoAgeApi(): PhenoAgeApi {
    return window.PhenoAge ?? api;
}

function biomarkerAt(index: number): PhenoBiomarker {
    const biomarker = currentPhenoAgeApi().biomarkers[index];
    if (!biomarker) throw new RangeError(`Missing pheno age biomarker at index ${index}.`);
    return biomarker;
}

// Liver: Albumin (index 1) and alkaline phosphatase (index 9).
function calculateLiverScore(markerValues: readonly number[]): number {
...
function calculateKidneyScore(markerValues: readonly number[]): number {
...
function calculateMetabolicScore(markerValues: readonly number[]): number {
...
function calculateInflammationScore(markerValues: readonly number[]): number {
    return markerValue(markerValues, 4) * biomarkerAt(4).coeff;
}

// Immune: WBC, lymphocytes, MCV, and RDW.
function calculateImmuneScore(markerValues: readonly number[]): number {
...
function calculatePhenoAge(markerValues: readonly number[]): number {
...
function calculateLiverPhenoAgeContributor(markerValues: readonly number[]): number {
    return currentPhenoAgeApi().calculateLiverScore(markerValues) * scalingFactor;
}

function calculateKidneyPhenoAgeContributor(markerValues: readonly number[]): number {
    return currentPhenoAgeApi().calculateKidneyScore(markerValues) * scalingFactor;
}

function calculateMetabolicPhenoAgeContributor(markerValues: readonly number[]): number {
    return currentPhenoAgeApi().calculateMetabolicScore(markerValues) * scalingFactor;
}

function calculateInflammationPhenoAgeContributor(markerValues: readonly number[]): number {
    return currentPhenoAgeApi().calculateInflammationScore(markerValues) * scalingFactor;
}

function calculateImmunePhenoAgeContributor(markerValues: readonly number[]): number {
    return currentPhenoAgeApi().calculateImmuneScore(markerValues) * scalingFactor;
}

const api: PhenoAgeApi = {
    biomarkers,
    parseInput,
    calculateLiverScore,
