...
function createAthleteSelectionController(options) {
    ...
    function clearCurrentAthleteSelectionIfInputChanged(value) {
        if (!currentAthlete || isAthleteInputValue(currentAthlete, value))
            return;
        currentAthlete = null;
    ...
    function hydrateStoredAthleteSelection() {
        ...
        if (!storedAthlete || !storedAthlete.Name)
            return false;
    ...
    function renderAthleteMatches() {
        ...
        closeAllLists();
        if (currentAthlete && isAthleteInputValue(currentAthlete, input.value))
            return false;
        if (!terms.length)
            return false;
        ...
        athletes.forEach((athlete) => {
            if (count >= maxItems)
                return;
            if (!getAthleteCanonicalName(athlete))
                return;
    ...
    function bind() {
        ...
        document.addEventListener("click", event => {
            if (autocompleteRootSelector
                ...
                && event.target.closest(autocompleteRootSelector))
                return;
            closeAllLists();
        });
        return api;
...
function preserveAppliedDiscountMetadata(offer, result) {
    ...
    if (!hasDiscountCode || !window.addActiveDiscountMetadataToPaymentOffer)
        return offer;
    try {
...
function createDashboardButton(label, className, href, beforeNavigate) {
    ...
    button.addEventListener("click", () => {
        if (typeof beforeNavigate === "function" && beforeNavigate(button) === false)
            return;
...
function renderDashboardActions(athlete, options) {
    ...
    return ready.catch(() => { }).then(() => {
        ...
        const challengeButton = createDashboardButton("<span class=\"dashboard-action-label flow-action__label\">Longevitymaxxing</span><i class=\"fas fa-calendar-check\" aria-hidden=\"true\"></i>", "option-button grey flow-action flow-action--secondary", "/longevitymaxxing", () => clearPendingPaymentOffer());
        if (isPro) {
            const phenoButton = createDashboardButton("<span class=\"dashboard-action-label flow-action__label\">Update Pheno&nbsp;Age</span><i class=\"fas fa-rocket\" aria-hidden=\"true\"></i>", "option-button grey flow-action flow-action--secondary", "/pheno-age?update=1", () => clearPendingPaymentOffer());
            const bortzButton = createDashboardButton("<span class=\"dashboard-action-label flow-action__label\">Update Bortz&nbsp;Age</span><i class=\"fas fa-rocket\" aria-hidden=\"true\"></i>", "option-button grey flow-action flow-action--secondary", "/bortz-age?update=1", () => clearPendingPaymentOffer());
            dynamicActions.append(challengeButton, phenoButton, bortzButton);
            refreshFlowActionDock();
            return;
        }
        const submitButton = createDashboardButton("<span class=\"dashboard-action-label flow-action__label\">Submit new results</span><i class=\"fas fa-rocket\" aria-hidden=\"true\"></i>", "option-button grey flow-action flow-action--secondary", "/pheno-age?update=1", () => clearPendingPaymentOffer());
        const goProDiscountResult = window.proDiscounts && typeof window.proDiscounts.buildDiscountBreakdown === "function"
        ...
        const goProPriceHtml = window.hasFreePass && window.hasFreePass()
            ...
            : goProDiscountResult
                ...
                : null;
        const goProButton = createDashboardButton(goProPriceHtml
            ...
            : "<span class=\"dashboard-action-label flow-action__label\">Go pro</span><i class=\"fas fa-bolt\" aria-hidden=\"true\"></i>", "option-button green flow-action", "/bortz-age?update=1", button => {
            const amountUsd = goProDiscountResult && Number.isFinite(goProDiscountResult.finalPriceUsd)
