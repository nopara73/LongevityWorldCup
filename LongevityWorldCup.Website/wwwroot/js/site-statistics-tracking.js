...
(function () {
    ...
    function flowFromPath() {
        ...
        if (path.includes("bortz-age") || path === "/bortz-age")
            return "bortz";
        if (path.includes("pheno-age") || path === "/pheno-age")
            return "pheno";
        if (path.includes("join-game") || path === "/join")
            return "onboarding";
        ...
        if (path.includes("proof-upload") || path === "/proofs")
            return "application";
        if (path.includes("longevitymaxxing"))
            return "challenge";
        return "site";
    }
    ...
    function source() {
        ...
        if (!host)
            return hasCampaignParams() ? "campaign" : "direct";
        if (isInternalReferrer(host))
            return "internal";
        if (isEmailReferrer(host))
            return "email";
        if (/google|bing|duckduckgo|yahoo|brave|search/i.test(host))
            return "search";
        if (/x\.com|twitter|facebook|instagram|threads|youtube|linkedin|reddit|slack/i.test(host))
            return "social";
        return "referral";
    }
    ...
    function withStatsSessionHeader(args) {
        const input = args[0];
        const suppliedInit = args[1];
        if (!args.length || typeof Headers === "undefined" || !isSameOriginFetch(input)) {
            return args;
        }
        const init = suppliedInit ? Object.assign({}, suppliedInit) : {};
        const headers = new Headers(init.headers || (typeof Request !== "undefined" && input instanceof Request ? input.headers : undefined));
        if (!headers.has(sessionHeader))
            headers.set(sessionHeader, getSessionId());
    ...
    function isRequiredCalculatorControl(target) {
    ...
    function safeFieldKey(el) {
        ...
        if (/email|name|token|note|why|media|link/i.test(key))
    ...
    function safeToken(value, maxLength) {
        ...
        if (!text)
            return "";
        if (/@|https?:\/\/|data:|secret|bearer|password|token/i.test(text))
            return "";
        return text
    ...
    function currentLeagueSlug() {
        ...
        if (filters)
            return safeToken(filters.split(",")[0], 64);
    ...
    function eventBoardClickMetadata(target) {
        ...
        const row = target.closest("tr.main-row");
        const href = link ? String(link.getAttribute("href") || "") : "";
        let targetKind = "event";
        if (link && link.classList.contains("view-all"))
            targetKind = "view_all";
        else if (link && (link.classList.contains("event-athlete-link") || href.startsWith("/athlete/")))
            targetKind = "athlete";
        else if (href.startsWith("/league/") || href.includes("view=") || href.includes("filters="))
            targetKind = "league";
        else if (link)
            targetKind = "link";
        const metadata = { targetKind };
        if (row && row.dataset.eventTypeName)
            metadata.eventType = safeToken(row.dataset.eventTypeName, 64);
        if (row && row.dataset.highlightSelection)
            metadata.highlightSelection = safeToken(row.dataset.highlightSelection, 64);
        if (row && row.dataset.primarySlug)
            metadata.primarySlug = safeToken(row.dataset.primarySlug, 64);
        return metadata;
    }
    ...
    function setupPageViews() {
        ...
        if (flowFromPath() === "challenge") {
            ...
            if (params.has("confirm"))
                track("challenge_confirmation_link_opened", { component: "challenge", outcome: "opened" });
            if (params.has("token"))
                track("challenge_participant_page_opened", { component: "challenge", outcome: "opened" });
            if (params.has("stop"))
                track("challenge_stop_email_clicked", { component: "challenge", outcome: "clicked" });
            return;
        }
    ...
    function setupCalculatorTracking() {
        ...
        function recordFieldCompletion(el, source) {
            ...
            if (!fieldHasRequiredValue(el) || completedFields.has(key))
                return false;
        ...
        function scanRequiredFields(source) {
            let changed = false;
            required.forEach(el => {
                if (recordFieldCompletion(el, source))
                    changed = true;
            });
            if (changed || completedFields.size > 0)
                recordRequiredProgress(source);
        }
        listen(form, "submit", () => {
        ...
        if (continueButton) {
            listen(continueButton, "click", () => {
                ...
                if (paymentOffer)
                    track("payment_offer_stored", { component: "handoff", outcome: "stored" });
                if (biomarkerData)
                    track("biomarker_handoff_stored", { component: "handoff", outcome: "stored" });
            }, { passive: true });
        }
        if (result) {
        ...
        if (rank) {
            const observer = new MutationObserver(() => {
                safe(() => {
                    if (rank.hidden || !rank.textContent.trim())
                        return;
    ...
    function setupChallengeTracking() {
        ...
        listen(document, "input", (event) => {
            const target = event.target;
            if (!(target instanceof HTMLInputElement) || (target.id !== "lmxPledgeCommitmentAmount" && target.id !== "lmxBlockedCommitmentAmount" && target.id !== "lmxEditCommitmentAmount"))
                return;
        ...
        listen(document, "submit", (event) => {
            ...
            if (!(form instanceof HTMLFormElement) || !form.classList.contains("lmx-checkin-card"))
                return;
        ...
        if (commitmentPanel && typeof MutationObserver === "function") {
            const trackCommitmentBlock = () => {
                if (commitmentPanel.classList.contains("lmx-hidden"))
                    return;
    ...
    function classifyFetch(url) {
        if (!url || url.includes("/api/site-statistics/event"))
            return null;
    ...
    listen(window, "error", (event) => {
        ...
        if (isIgnoredClientErrorMessage(message))
            return;
