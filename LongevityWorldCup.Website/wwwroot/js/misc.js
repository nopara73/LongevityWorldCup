(() => {
    if (window.__lwcPlayFlowScrollInitialized)
        return;
...
window.calculateAgeAtDate = function (birthDate, atDate) {
    if (!(birthDate instanceof Date))
        throw new Error("Invalid input: birthDate must be a Date object");
    if (!(atDate instanceof Date))
        throw new Error("Invalid input: atDate must be a Date object");
    if (Number.isNaN(birthDate.getTime()))
        throw new Error("Invalid date of birth.");
    if (Number.isNaN(atDate.getTime()))
        throw new Error("Invalid blood draw date.");
    if (birthDate > atDate)
...
window.optimizeImageClient = async function (dataUri, options) {
    ...
    const encode = async (maxSize, quality) => {
        ...
        return await new Promise(resolve => canvas.toBlob(resolve, targetContentType, quality));
    };
...
window.extractApplicationErrorMessage = function (text, fallback) {
    ...
    if (/^(?:<!doctype\s+html\b|<html[\s>])/i.test(raw))
        return fallback || 'Request failed';
...
window.buildApplicationSubmissionReport = function (applicantData, submissionId, phase, submissionKind, error) {
    ...
    const proofs = Array.isArray(data.proofPics) ? data.proofPics : [];
...
function isHypotheticalRankNearbyItem(value) {
    if (!isRecord(value))
        return false;
...
function isHypotheticalRankResult(value) {
    if (!isRecord(value)
