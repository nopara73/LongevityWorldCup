...
function applyCap(value, feature) {
    if (feature.cap === undefined || feature.capMode === undefined)
        return value;
    if (feature.capMode === "floor")
        return Math.max(value, feature.cap);
    return Math.min(value, feature.cap);
}
function currentBortzAgeApi() {
    return window.BortzAge ?? api;
}
function calculateBAA(values) {
    const activeFeatures = currentBortzAgeApi().features;
    if (!values || values.length !== activeFeatures.length)
        return Number.NaN;
    let sum = 0;
...
function calculateBortzAgeFromBAA(chronologicalAgeYears, baa) {
...
function calculateBortzAge(chronologicalAgeOrValues, rawValuesInFeatureOrder) {
    let values = rawValuesInFeatureOrder;
    let chronologicalAge = chronologicalAgeOrValues;
    ...
    if (!Number.isFinite(baa))
        return Number.NaN;
    return activeApi.calculateBortzAgeFromBAA(typeof chronologicalAge === "number" ? chronologicalAge : Number.NaN, baa);
}
const api = {
    features,
