...
function hasPerfectGuessMarker() {
    try {
        if (localStorage.getItem(PERFECT_GUESS_KEY) === "1")
            return true;
        const parsed = JSON.parse(localStorage.getItem("gmaAllGuesses") || "{}");
...
function getExactGuessAthleteNames(currentAthlete) {
    try {
        ...
        const normalizedSelectedSlug = selectedSlug || (slugify && selectedName ? slugify(selectedName, true) : "");
...
function weightForAgeReduction(category, place) {
    ...
    if (category === "amateur")
        return 100;
    if (category === "global")
        return [100, 50, 40][place - 1] ?? 0;
    if (category === "division")
        return [100, 50, 20][place - 1] ?? 0;
...
function weightForBadge(badge) {
    ...
    if (label === "First applicants") {
        ...
        if (place === 3)
            return 80;
        if (place !== null && place >= 4 && place <= 10)
            return 70;
        return 0;
    }
    if (label === "Pregnancy")
    ...
    if (isSeasonBadgeLabel(label)) {
        ...
        if (place !== null && place >= 4 && place <= 10)
            return 70;
        if (place !== null && place >= 11 && place <= 20)
            return 60;
        return 0;
    }
    return 0;
...
function formatCompactDiscountLine(component) {
    ...
    if (component?.kind === "leaderboard")
        return `${percent}% leaderboard`;
    if (component?.kind === "personalLink")
        return `${percent}% personal page`;
    if (component?.kind === "perfectGuess")
        return `${percent}% perfect guess`;
...
function describeDiscountReason(component) {
    if (!component)
        return "eligible achievements";
    if (component.kind === "leaderboard")
        return "being on the leaderboard";
    if (component.kind === "discountCode")
        return component.code ? `using discount code ${component.code}` : "using a discount code";
    if (component.kind === "personalLink")
        return "linking your personal page";
    if (component.kind === "perfectGuess")
        return "guessing an athlete's age perfectly";
...
function describeServerBadgeReason(badge) {
    ...
    if (isSeasonBadgeLabel(label)) {
        ...
        if (place === 1)
            return `winning ${seasonTag}`;
        if (place === 2)
            return `a 2nd-place finish in ${seasonTag}`;
        if (place === 3)
            return `a 3rd-place finish in ${seasonTag}`;
        if (place !== null && place >= 4 && place <= 10)
            return `a top-10 finish in ${seasonTag}`;
        if (place !== null && place >= 11 && place <= 20)
            return `a top-20 finish in ${seasonTag}`;
        return `a seasonal finish in ${seasonTag}`;
    }
    if (label === "First applicants") {
        if (place === 1)
            return "being the first applicant to join";
        if (place === 2)
            return "being the second applicant to join";
        if (place === 3)
            return "being the third applicant to join";
        if (place !== null && place >= 4 && place <= 10)
            return "being among the first 10 applicants";
        return "being an early applicant";
    }
    if (label === "Podcast")
        return "appearing on the Longevity World Cup podcast";
    if (label === "Host")
        return "hosting the Longevity World Cup";
    if (label === "Perfect application")
        return "submitting a perfect application";
    if (label === "Most submissions")
        return "having the most submissions";
    if (label === "≥2 submissions")
        return "submitting at least two results";
    if (label === "Pheno Age best improvement")
        return "having the best pheno age improvement";
    if (label === "Bortz Age best improvement")
        return "having the best bortz age improvement";
    if (label === "Pregnancy")
        return "holding the Pregnancy badge";
    ...
    if (label === "Age reduction" && place) {
        if (category === "Amateur")
            return `while #${place} in the Amateur League`;
        if (category === "Global")
            return `while #${place} in the Ultimate League`;
        if (category === "Division" && leagueValue)
            return `while #${place} in the ${leagueValue} League`;
        if (category === "Generation" && leagueValue)
            return `while #${place} in the ${leagueValue} League`;
        if (category === "Exclusive" && leagueValue)
            return `while #${place} in the ${leagueValue} League`;
...
function getDiscountIconClass(component) {
    if (!component || !component.isBadge)
        return null;
    if (component.kind === "personalLink")
        return "fa-link";
    if (component.kind === "perfectGuess")
...
function getDiscountBadgeBackground(component) {
    ...
    if (component.kind === "personalLink")
        return BADGE_BG_PERSONAL;
    if (component.kind === "perfectGuess")
        return BADGE_BG_BLACK;
    if (component.kind === "serverBadge") {
