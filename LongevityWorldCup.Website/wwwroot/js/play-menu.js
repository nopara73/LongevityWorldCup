(function () {
    ...
    function getFlowReady() {
        return Promise.resolve(window.modulesReady || undefined)
            ...
            if (!flow)
                throw new Error('Athlete flow module did not load.');
            return flow;
        });
    }
    function requirePlayFlow() {
        if (!flow)
            throw new Error('Athlete flow module is not initialized.');
        return flow;
    }
    function requireAthleteSelection() {
        if (!athleteSelection)
            throw new Error('Athlete selection is not initialized.');
        return athleteSelection;
    }
    function isPlayPanelName(value) {
    ...
    function completePlayPanelTransition(panelName, transitionRunId) {
        if (transitionRunId !== playPanelTransitionRunId)
            return;
    ...
    function showPanelByName(panelName, options = {}) {
        ...
        if (!shouldAnimate) {
            commitPanel();
            if (panelName === 'start')
                startPlayStartIntro({ restart: hasRenderedPlayPanel });
        ...
        activePanelTransitionTimer = window.setTimeout(() => {
            ...
            if (panelName === 'start')
                startPlayStartIntro({ restart: true });
            completePlayPanelTransition(panelName, transitionRunId);
        }, 180);
    }
    ...
    function showPlayPanel(requestedPanelName, options = {}) {
        ...
        if (panelName === 'selection') {
            ...
            if (!hydratedStoredAthlete && athleteSelection.hasPendingSavedSelection()) {
                ...
                waitForPlayPanelPreparation(athleteSelection.loadAthletes({ savedSelectionTransition: false }))
        ...
        if (panelName === 'dashboard') {
            ...
            if (!athlete || !dashboardTitle || !dashboardPicture || !dynamicActions)
                return;
    ...
    function initializePlayHistory() {
        ...
        window.history.replaceState(createPlayHistoryState(resolved.panelName), '', getPlayUrl(resolved.panelName, { preserveCurrentUrlParts: true }));
    }
    ...
    function getCheckoutQuerySuffix() {
        ...
        if (freePassValue !== null)
            params.set('freepass', freePassValue);
        if (discountValue !== null)
            params.set('discount', discountValue);
    ...
    function startProApplication(retryButton) {
        ...
        if (!flow.setPendingPaymentOffer(paymentOffer, retryButton))
            return;
        window.location.href = `/bortz-age${getCheckoutQuerySuffix()}`;
    }
    function renderJoinFreePassPricing() {
        if (!window.hasFreePass || !window.hasFreePass())
            return false;
        ...
        if (amateurPrice)
            amateurPrice.textContent = 'free';
        if (proEntryPrice)
            proEntryPrice.innerHTML = '<span class="pro-new-price">free</span>';
        if (breakdown)
    ...
    function renderJoinPricing() {
        ...
        const breakdown = document.getElementById('joinProDiscountBreakdown');
        const proEntryPrice = document.getElementById('joinProEntryPrice');
        if (!note || !breakdown || !proEntryPrice)
            return;
        breakdown.classList.toggle('pro-discount-breakdown--with-badges', Array.isArray(result.components) && result.components.some(component => component && component.isBadge));
