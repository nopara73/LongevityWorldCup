...
(function () {
    ...
    function readSharedWindowApi() {
        const api = {};
        if ("PhenoAge" in window && isPhenoAgeApi(window.PhenoAge))
            api.PhenoAge = window.PhenoAge;
        if ("BortzAge" in window && isBortzAgeApi(window.BortzAge))
            api.BortzAge = window.BortzAge;
        if ("calculateAgeAtDate" in window && isAgeAtDateFunction(window.calculateAgeAtDate))
            api.calculateAgeAtDate = window.calculateAgeAtDate;
        if ("getGeneration" in window && isGenerationFunction(window.getGeneration))
            api.getGeneration = window.getGeneration;
        if ("slugifyName" in window && isSlugifyFunction(window.slugifyName))
            api.slugifyName = window.slugifyName;
        return api;
    }
    function isPhenoAgeApi(value) {
        return hasProperties(value, "calculatePhenoAge") && typeof value.calculatePhenoAge === "function";
    }
    function isBortzAgeApi(value) {
        return hasProperties(value, "calculateBortzAge") && typeof value.calculateBortzAge === "function";
    }
    function isAgeAtDateFunction(value) {
        return typeof value === "function";
    }
    function isGenerationFunction(value) {
        return typeof value === "function";
    }
    function isSlugifyFunction(value) {
        return typeof value === "function";
    }
    function requiredElement(id, expected) {
    ...
    function optionalElement(id, expected) {
        const element = document.getElementById(id);
    ...
    function isParticipantTab(value) {
    ...
    function wireForms() {
        ...
        profilePictureInput.addEventListener("change", async () => {
            ...
            if (file)
                await uploadProfilePicture(file, profilePictureInput);
        });
    ...
    function renderMetrics(state) {
        ...
        if (boardSection)
            boardSection.classList.toggle("signup-roster", preStartSignup);
        if (preStartSignup) {
    ...
    function setParticipantTab(tab, manual) {
        if (!isParticipantTab(tab) || !participantState)
            return;
        if (isParticipantTabLocked(tab, participantState))
            return;
    ...
    function getParticipantTabPanel(tab) {
        if (tab === "checkin")
            return document.getElementById("lmxCheckinPanel");
        if (tab === "profile")
            return document.getElementById("lmxEditForm");
        if (tab === "home")
            return document.getElementById("lmxHomePanel");
        return null;
    }
    function handleParticipantTabKeydown(event, button) {
        const currentParticipantState = participantState;
        if (!currentParticipantState || hasCommitmentBlock(currentParticipantState))
            return;
        ...
        if (!isParticipantTab(requestedTab))
            return;
    ...
    async function saveCommitmentAmountFromPanel(inputId, button, successMessage) {
        if (!accessToken || !participantState)
            return;
        const currentParticipantState = participantState;
    ...
    async function payCommitment(button) {
        ...
        await withStandaloneButton(button, button?.dataset.busyText || "Creating invoice...", async () => {
            ...
            if (!checkoutLink)
                throw new Error("The payment invoice did not return a usable checkout link.");
            if (checkoutWindow) {
    ...
    function startCommitmentPaymentPolling() {
        ...
        COMMITMENT_PAYMENT_POLL_DELAYS_MS.forEach((delay, index) => {
            window.setTimeout(() => {
                if (run !== commitmentPaymentPollRun || !hasCommitmentBlock(participantState))
                    return;
    ...
    function formatCallCountdown(startsAtUtc) {
        ...
        if (remainingMinutes <= 0)
            return { label: "Live now", value: "Join" };
        if (remainingMinutes < 60)
            return { label: "Starts in", value: `${remainingMinutes}m` };
        const days = Math.floor(remainingMinutes / 1440);
        const hours = Math.floor((remainingMinutes % 1440) / 60);
        const minutes = remainingMinutes % 60;
        if (days > 0)
            return { label: "Starts in", value: hours > 0 ? `${days}d ${hours}h` : `${days}d` };
    ...
    function renderBoard(state) {
        ...
        const rows = leaderboardRows.visible.map((row, index) => {
            ...
            const cells = (row.cells || []).map(cell => {
                if (!cell.checkedIn)
                    return `<div class="lmx-cell empty" data-day="${escAttr(cell.challengeDay)}" title="Day ${cell.challengeDay}"></div>`;
                if (cell.countsForScore === false) {
    ...
    function habitCellTitle(cell, score, breakdown) {
        if (!breakdown.length)
            return `Day ${cell.challengeDay}: ${score}`;
    ...
    function practiceCellTitle(cell, breakdown) {
        if (!breakdown.length)
            return `Day ${cell.challengeDay}: practice check-in`;
        const pieces = breakdown.map(item => `${item.label} ${item.value}/2`);
        const missed = breakdown.filter(item => item.value < 2).map(item => item.label.toLowerCase());
    ...
    function habitMarkClass(value) {
        if (value >= 2)
            return "lmx-habit-mark full";
        if (value > 0)
            return "lmx-habit-mark partial";
    ...
    function scrollBoardToLatestDay() {
        ...
        if (window.ResizeObserver && boardScrollObservedElement !== scroller) {
            if (boardScrollObserver)
                boardScrollObserver.disconnect();
            ...
            if (board)
                boardScrollObserver.observe(board);
        }
    }
    function scrollDashboardToLatestDay() {
        ...
        if (window.ResizeObserver && dashboardScrollObservedElement !== scroller) {
            if (dashboardScrollObserver)
                dashboardScrollObserver.disconnect();
            ...
            if (dashboard)
                dashboardScrollObserver.observe(dashboard);
        }
    }
    ...
    function renderCheckIns(days, containerId = "lmxCheckinList", recentRemarks = []) {
        ...
        if (previousForm)
            revokePendingNotePhotoUrls(checkInDayKey(previousForm));
    ...
    function pickActiveCheckInDay(days) {
        ...
        if (!fallback)
            throw new Error("At least one eligible check-in day is required.");
    ...
    function setPendingNotePhotos(form, files) {
        ...
        if (photos.length)
            pendingNotePhotos.set(key, photos);
        else
    ...
    function renderSelectedNotePhotoPreviews(form) {
        ...
        if (urls.length)
            pendingNotePhotoUrls.set(key, urls);
    ...
    function selectQuoteBucket(draft) {
        ...
        if (values.every(item => item.value === 2))
            return null;
    ...
    function updateCheckInQuoteDialogRank(quote, bestRank, token) {
        ...
        if (!dialog || dialog.hidden || dialog.dataset.quoteToken !== token)
            return;
        const source = dialog.querySelector("#lmxQuoteDialogSource");
        if (source)
            source.innerHTML = renderCheckInQuoteSourceHtml(quote, bestRank);
    ...
    async function submitCheckIn(form) {
        ...
        await withButton(form.querySelector("button[type='submit']"), async () => {
            ...
            renderAll();
            if (quoteBucket)
                void showRandomCheckInQuote(quoteBucket);
        }, "Saving...");
    }
    ...
    function collectCheckInDraft(form) {
        const readHabit = (key) => {
            const pressed = form.querySelector(`.lmx-question[data-key="${key}"] button[aria-pressed="true"]`);
    ...
    function hasCheckInChanged(form) {
        if (!form || form.dataset.saved !== "true")
            return true;
        const draft = collectCheckInDraft(form);
        if (getPendingNotePhotos(form).length > 0)
            return true;
        if ((form.dataset.originalNote || "") !== draft.note)
            return true;
    ...
    function wireCommitmentAmountValidation(inputId) {
        const input = optionalInput(inputId);
        if (!input || input.dataset.commitmentValidationWired)
            return;
    ...
    function markCommitmentAmountInvalid(input, message, report = false) {
        if (!input)
            return;
    ...
    function findAthleteForParticipant(row) {
        if (!row || !row.athleteUrl || !athleteDirectory.length)
            return null;
    ...
    function isAthleteRecord(value) {
        return typeof value === "object" && value !== null && !Array.isArray(value);
    }
    function loadAthleteDirectory() {
        if (athleteDirectoryPromise)
    ...
    function assignQuoteBestRankCandidates(athletes) {
        ...
        assignQuoteRankedBestCandidates(sortedQuoteRankableAthletes(rows, hasQuoteBortzRankData, compareQuoteAthleteRank), () => ({
        ...
        assignQuoteRankedBestCandidates(sortedQuoteRankableAthletes(rows, athlete => Number.isFinite(athlete.ageReduction), compareQuoteAthleteRankPhenoOnly), () => ({
        ...
        assignQuoteRankedBestCandidates(sortedQuoteRankableAthletes(rows, athlete => !hasQuoteBortzRankData(athlete), compareQuoteAthleteRankPhenoOnly), () => ({
        ...
        assignQuoteRankedBestCandidates(sortedQuoteRankableAthletes(rows, athlete => Number.isFinite(athlete.phenoAgeImprovement), compareQuoteAthleteRankPhenoImprovement), () => ({
        ...
        assignQuoteRankedBestCandidates(sortedQuoteRankableAthletes(rows, athlete => Number.isFinite(athlete.bortzAgeImprovement), compareQuoteAthleteRankBortzImprovement), () => ({
        ...
        assignQuoteRankedBestCandidates(sortedQuoteRankableAthletes(rows, athlete => athlete.crowdCount >= CROWD_AGE_LEADERBOARD_MINIMUM_GUESS_COUNT && Number.isFinite(athlete.crowdAgeReduction), compareQuoteAthleteRankCrowdAge), () => ({
        ...
        assignQuoteRankedBestCandidates(orderByQuoteNumberDesc(rows.filter(athlete => Number.isFinite(athlete.ageReductionPercent)), athlete => athlete.ageReductionPercent), () => ({
            leagueName: "Pheno pace of aging",
            leagueLabel: "Pheno pace of aging ranking",
            leagueType: "pheno-pace",
            tiePriority: 14
        }));
        assignQuoteRankedBestCandidates(orderByQuoteNumberAsc(rows.filter(athlete => quoteBortzPace(athlete) !== null), quoteBortzPace), () => ({
        ...
        const addGroupedRanks = (values, predicate, comparator, candidateFactory) => {
            values.filter(Boolean).forEach(value => {
                assignQuoteRankedBestCandidates(sortedQuoteRankableAthletes(rows, athlete => predicate(athlete, value), comparator), athlete => candidateFactory(athlete, value));
            });
        };
        const divisions = [...new Set(rows.map(athlete => athlete.division).filter(Boolean))];
        addGroupedRanks(divisions, (athlete, division) => athlete.division === division, compareQuoteAthleteRank, (_athlete, division) => ({
            leagueName: division,
        ...
        addGroupedRanks(generations, (athlete, generation) => athlete.generation === generation, compareQuoteAthleteRank, (_athlete, generation) => ({
        ...
        divisions.forEach(division => {
            generations.forEach(generation => {
                assignQuoteRankedBestCandidates(sortedQuoteRankableAthletes(rows, athlete => athlete.division === division && athlete.generation === generation, compareQuoteAthleteRank), () => ({
        ...
        addGroupedRanks(exclusiveLeagues, (athlete, exclusiveLeague) => athlete.exclusiveLeague === exclusiveLeague, compareQuoteAthleteRank, (_athlete, exclusiveLeague) => ({
    ...
    function addQuoteBestRankCandidate(athlete, candidate) {
        ...
        if (!normalized.leagueName && !normalized.leagueLabel)
            return;
    ...
    function computeQuoteAthleteBestRank(athlete) {
        if (!athlete || !("bestRankCandidates" in athlete))
            return null;
    ...
    function wireAthleteSelector(input, athletes) {
        if (athleteSelectors.has(input.id))
            return;
        let currentFocus = -1;
        ...
        const selector = {
            ...
            getPayload() {
                ...
                const normalized = normalizeAthleteSlug(raw);
                const match = athletes.find(a => a.name.toLowerCase() === raw.toLowerCase() ||
                    a.legalName.toLowerCase() === raw.toLowerCase() ||
        ...
        document.addEventListener("click", event => {
            if (!(event.target instanceof Node) || !input.closest(".lmx-athlete-selector")?.contains(event.target))
                closeList();
        });
        ...
        function renderSuggestions(showInitial = false) {
            const query = input.value.trim().toLowerCase();
            const terms = query.split(/\s+/).filter(Boolean);
            closeList();
            if (!terms.length && !showInitial)
                return;
    ...
    function getAthleteSelectorDisplayName(id) {
        const selector = athleteSelectors.get(id);
        if (selector)
            return selector.getSelectedName();
        const input = optionalInput(id);
        return input?.dataset.athleteName || input?.value.trim() || "";
    }
    ...
    function normalizeAthleteSlug(value) {
        ...
        raw = raw.replace(/^\/+|\/+$/g, "");
        if (raw.toLowerCase().startsWith("athlete/"))
            raw = raw.slice("athlete/".length);
        return raw
            .trim()
            .replace(/_/g, "-")
            .toLowerCase()
    ...
    function isParticipantNote(value) {
    ...
    function isLeaderboardRow(value) {
    ...
    function isParticipantCall(value) {
    ...
    function isParticipantSummary(value) {
    ...
    function isCommitmentState(value) {
    ...
    function isCommitmentTrendGuidance(value) {
    ...
    function isParticipantState(value) {
    ...
    async function getJson(url) {
        ...
        if (url === `${API}/state` && !isPublicState(data))
    ...
    async function postJson(url, payload) {
        ...
        const data = await readJsonResponse(response, url);
        if ((url === `${API}/signup` || url === `${API}/resend`) && !isSignupResult(data))
            throw invalidApiResponse(url);
        if (url === `${API}/confirm` && !isAccessResult(data))
            throw invalidApiResponse(url);
    ...
    async function requestJson(url, options) {
        ...
        try {
            ...
            if (hasStringProperty(err, "name") && err.name === "AbortError")
    ...
    function isAuthFailure(err) {
        if (!(err instanceof Error) || !hasProperties(err, "status"))
            return false;
    ...
    function openTimeZonePicker(picker) {
        document.querySelectorAll("[data-timezone-picker].open").forEach(openPicker => {
            if (openPicker !== picker)
                closeTimeZonePicker(openPicker);
        });
    ...
    function syncTimeZonePicker(picker) {
        ...
        if (display)
            display.textContent = timeZoneDisplayName(value);
        if (offset)
            offset.textContent = timeZoneOffsetLabel(value);
    }
    ...
    function handleTimeZoneSearchKeydown(event, picker) {
        ...
        if (event.key === "Enter") {
            ...
            if (active)
                chooseTimeZone(picker, active.dataset.timeZone || "UTC");
            return;
        }
    ...
    function getTimeZoneCountryCodes() {
        if (!timeZoneCountryCodes) {
            timeZoneCountryCodes = new Map(TIME_ZONE_COUNTRY_DATA.split("|").map(serializedEntry => {
                ...
                const mapEntry = [zone || "", String(codes || "").split(",").filter(Boolean)];
                return mapEntry;
            }));
        }
    ...
    function pendingCallTimeLabel(_callSelectionClosesAtUtc, _timeZoneId) {
