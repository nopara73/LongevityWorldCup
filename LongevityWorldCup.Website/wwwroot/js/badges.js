...
function getCat(b) { return String(b.LeagueCategory || b.Category || 'Global'); }
...
function pickBackgroundForServerBadge(b) {
    ...
    if (label === 'Pheno Age – lowest')
        return LEGACY_BG.pheno;
    if (label === 'Bortz Age – lowest')
        return LEGACY_BG.bortz;
    const medalLike = label === 'Age reduction' ||
        (typeof label === 'string' && label.startsWith('Crowd – '));
    ...
    if (label === 'Podcast')
        return LEGACY_BG.black;
    if (label === 'Best domain – liver')
        return LEGACY_BG.liver;
    if (label === 'Best domain – kidney')
        return LEGACY_BG.kidney;
    if (label === 'Best domain – metabolic')
        return LEGACY_BG.metabolic;
    if (label === 'Best domain – inflammation')
        return LEGACY_BG.inflammation;
    if (label === 'Best domain – immune')
        return LEGACY_BG.immune;
    if (label === 'Best domain – vitamin D')
        return LEGACY_BG.vitaminD;
    if (label === 'Pheno pace of aging')
        return LEGACY_BG.phenoPace;
    if (label === 'Bortz pace of aging')
        return LEGACY_BG.bortzPace;
...
function makeUnformattedTooltipFromServerBadge(b, athlete, opts) {
    const label = getLabel(b);
    const place = getPlace(b);
    const cat = (getCat(b) || '').toLowerCase();
    const val = getVal(b);
    ...
    if (label === 'Age reduction') {
        if (cat === 'global') {
            if (place === 1)
                return 'Ultimate Lifeform: #1 in the Ultimate League';
            if (place === 2)
                return 'Near Immortal: #2 in the Ultimate League';
            if (place === 3)
                return 'Third and threatening: #3 in the Ultimate League';
            return 'Age reduction';
        }
        if (cat === 'amateur') {
            if (place === 1)
                return 'Upstart: #1 in Amateur League';
            if (place === 2)
                return 'Challenger: #2 in Amateur League';
            if (place === 3)
                return 'Harbinger of Change: #3 in Amateur League';
        ...
        if (cat === 'division') {
            if (place === 1) {
                ...
                if (v === "men's")
                    return "The Alpha Male: #1 in Men's League";
                if (v === "women's")
                    return "The Empress: #1 in Women's League";
                if (v === "open")
                    return 'The Machine: #1 in Open League';
            }
        ...
        if (cat === 'generation') {
            if (place === 1) {
                ...
                if (g === 'silent generation')
                    return 'The Grandmaster: #1 in Silent Generation League';
                if (g === 'baby boomers')
                    return 'The Iron Throne: #1 in Baby Boomers League';
                if (g === 'gen x')
                    return 'The Last Ronin: #1 in Gen X League';
                if (g === 'millennials')
                    return 'The Chosen One: #1 in Millennials League';
                if (g === 'gen z')
                    return 'The Meme Lord: #1 in Gen Z League';
                if (g === 'gen alpha')
                    return 'The Singularity: #1 in Gen Alpha League';
            }
            return `#${place} in ${val} League`;
        }
        if (cat === 'exclusive') {
    ...
    if (label === 'Chronological age – oldest' && place) {
        if (suppressValues) {
            if (place === 1)
                return 'Yoda: chronologically oldest';
            if (place === 2)
                return 'Master Roshi: chronologically 2nd oldest';
            if (place === 3)
                return 'Mr. Miyagi: chronologically 3rd oldest';
        }
        ...
        if (place === 1)
            return `Yoda: chronologically oldest (age: ${ageText} years)`;
        if (place === 2)
            return `Master Roshi: chronologically 2nd oldest (age: ${ageText})`;
        if (place === 3)
            return `Mr. Miyagi: chronologically 3rd oldest (age: ${ageText})`;
    }
    if (label === 'Chronological age – youngest' && place) {
        if (suppressValues) {
            if (place === 1)
                return 'Son Goten: chronologically youngest';
            if (place === 2)
                return 'Son Gohan: chronologically 2nd youngest';
            if (place === 3)
                return 'Son Goku: chronologically 3rd youngest';
        }
        const age = (athlete?.chronologicalAge ?? athlete?.ChronoAge ?? athlete?.chronological_age);
        const hasAge = Number.isFinite(Number(age));
        const ageText = hasAge ? Number(age).toFixed(1) : '';
        ...
        if (place === 1)
            return `Son Goten: chronologically youngest (age: ${ageText} years)`;
        if (place === 2)
            return `Son Gohan: chronologically 2nd youngest (age: ${ageText})`;
        if (place === 3)
            return `Son Goku: chronologically 3rd youngest (age: ${ageText})`;
    }
    if (label === 'Pheno Age – lowest' && place) {
        if (suppressValues) {
            if (place === 1)
                return 'Peter Pan: biologically youngest according to pheno age';
            if (place === 2)
                return 'Dorian Gray: biologically 2nd youngest according to pheno age';
            if (place === 3)
                return 'Benjamin Button: biologically 3rd youngest according to pheno age';
        }
        ...
        if (place === 1)
            return `Peter Pan: biologically youngest according to pheno age: ${phText} years old`;
        if (place === 2)
            return `Dorian Gray: biologically 2nd youngest according to pheno age: ${phText} years old`;
        if (place === 3)
            return `Benjamin Button: biologically 3rd youngest according to pheno age: ${phText} years old`;
    }
    if (label === 'Bortz Age – lowest' && place) {
        if (suppressValues) {
            if (place === 1)
                return 'Peter Pan: biologically youngest according to bortz age';
            if (place === 2)
                return 'Dorian Gray: biologically 2nd youngest according to bortz age';
            if (place === 3)
                return 'Benjamin Button: biologically 3rd youngest according to bortz age';
        }
        ...
        if (place === 1)
            return `Peter Pan: biologically youngest according to bortz age: ${baText} years old`;
        if (place === 2)
            return `Dorian Gray: biologically 2nd youngest according to bortz age: ${baText} years old`;
        if (place === 3)
            return `Benjamin Button: biologically 3rd youngest according to bortz age: ${baText} years old`;
    }
    if (label === 'Pheno pace of aging' && place) {
        if (place === 1)
            return 'Time Bender: best pheno pace of aging';
        if (place === 2)
            return 'Slow Clock: 2nd best pheno pace of aging';
        if (place === 3)
            return 'Time Hacker: 3rd best pheno pace of aging';
    }
    if (label === 'Bortz pace of aging' && place) {
        if (place === 1)
            return 'Time Bender: best bortz pace of aging';
        if (place === 2)
            return 'Slow Clock: 2nd best bortz pace of aging';
        if (place === 3)
            return 'Time Hacker: 3rd best bortz pace of aging';
    }
    if (label === 'Most submissions') {
        if (suppressValues)
            return 'The Submittinator: most tests submitted';
    ...
    if (typeof label === 'string' && label.startsWith('Crowd – ')) {
        if (label.endsWith('most guessed') && place) {
            if (suppressValues) {
                if (place === 1)
                    return 'Popular AF: most age guesses received';
                if (place === 2)
                    return 'Pretty damn popular: 2nd most age guesses received';
                if (place === 3)
                    return 'Shockingly popular: 3rd most age guesses received';
            }
            const count = (athlete?.crowdCount ?? athlete?.CrowdCount ?? 0);
            if (place === 1)
                return `Popular AF: most age guesses received (${count})`;
            if (place === 2)
                return `Pretty damn popular: 2nd most age guesses received (${count})`;
            if (place === 3)
                return `Shockingly popular: 3rd most age guesses received (${count})`;
        }
        if (label.includes('age gap') && place) {
            if (suppressValues) {
                if (place === 1)
                    return 'Skin Trafficker: perceived younger';
                if (place === 2)
                    return 'Wrinkle Launderer: perceived younger';
                if (place === 3)
                    return 'Collagen Smuggler: perceived younger';
            }
            ...
            if (place === 1)
                return `Skin Trafficker: perceived ${gapText} ${yearWord} younger`;
            if (place === 2)
                return `Wrinkle Launderer: perceived ${gapText} ${yearWord} younger`;
            if (place === 3)
                return `Collagen Smuggler: perceived ${gapText} ${yearWord} younger`;
        }
        if (label.endsWith('lowest crowd age') && place) {
            if (suppressValues) {
                if (place === 1)
                    return 'Baby Boss: youngest looking';
                if (place === 2)
                    return 'Lullaby Lord: 2nd youngest looking';
                if (place === 3)
                    return 'Diaper Don: 3rd youngest looking';
            }
            const cr = Number(athlete?.crowdAge ?? athlete?.CrowdAge ?? 0);
            const ageText = Number.isFinite(cr) ? cr.toFixed(1) : '';
            const yearWord = ageText === '1.0' ? 'year' : 'years';
            if (place === 1)
                return `Baby Boss: youngest looking (crowd age: ${ageText} ${yearWord})`;
            if (place === 2)
                return `Lullaby Lord: 2nd youngest looking (crowd age: ${ageText} ${yearWord})`;
            if (place === 3)
                return `Diaper Don: 3rd youngest looking (crowd age: ${ageText} ${yearWord})`;
        }
    }
    if (label === 'Pheno Age best improvement') {
        if (suppressValues)
    ...
    if (label === 'Bortz Age best improvement') {
        ...
        let delta = null;
        if (athlete && typeof athlete.bortzAgeDifference === 'number')
            delta = athlete.bortzAgeDifference;
        else if (athlete && typeof athlete.BortzAgeDiffFromBaseline === 'number')
            delta = athlete.BortzAgeDiffFromBaseline;
        if (Number.isFinite(delta)) {
            const years = Number(delta).toFixed(1);
    ...
    if (typeof label === 'string' && label.startsWith('Best domain')) {
        if (suppressValues) {
            if (label === 'Best domain – liver')
                return 'Liver King: top liver profile';
            if (label === 'Best domain – kidney')
                return 'Kidney Overlord: top kidney profile';
            if (label === 'Best domain – metabolic')
                return 'Metabolic Machine: top metabolic profile';
            if (label === 'Best domain – inflammation')
                return 'Inflammation Whisperer: top inflammation profile';
            if (label === 'Best domain – immune')
                return 'Pathogen Punisher: top immune profile';
            if (label === 'Best domain – vitamin D')
                return 'Sun God: top vitamin D profile';
        }
        ...
        if (typeof window.getBestDomainBiomarkerTooltip === 'function' && Array.isArray(bortz) && bortz.length >= 22) {
            ...
            if (biomarkerPart) {
                if (label === 'Best domain – liver')
                    return 'Liver King: top liver profile ' + biomarkerPart;
                if (label === 'Best domain – kidney')
                    return 'Kidney Overlord: top kidney profile ' + biomarkerPart;
                if (label === 'Best domain – metabolic')
                    return 'Metabolic Machine: top metabolic profile ' + biomarkerPart;
                if (label === 'Best domain – immune')
                    return 'Pathogen Punisher: top immune profile ' + biomarkerPart;
                if (label === 'Best domain – inflammation')
                    return 'Inflammation Whisperer: top inflammation profile ' + biomarkerPart;
                if (label === 'Best domain – vitamin D')
                    return 'Sun God: top vitamin D profile ' + biomarkerPart;
            }
        }
        ...
        if (Array.isArray(best) && best.length >= 10) {
            ...
            if (label === 'Best domain – inflammation')
                return `Inflammation Whisperer: top inflammation profile (CRP ${crp} mg/L)`;
            ...
            if (label === 'Best domain – liver')
                return `Liver King: top liver profile (albumin ${alb} g/L, ALP ${alp} U/L)`;
            if (label === 'Best domain – kidney')
                return `Kidney Overlord: top kidney profile (creatinine ${creat} µmol/L)`;
            if (label === 'Best domain – metabolic')
                return `Metabolic Machine: top metabolic profile (glucose ${glu} mmol/L)`;
            if (label === 'Best domain – immune')
                return `Pathogen Punisher: top immune profile (WBC ${wbc} 10³ cells/µL, lymphocyte ${lym}%, MCV ${mcv} fL, RDW ${rdw}%)`;
        }
        if (label === 'Best domain – vitamin D')
            return 'Sun God: top vitamin D profile';
    }
    if (label === 'First applicants') {
        if (place === 1)
            return 'Athlete Zero: 1st athlete to join the Longevity World Cup';
        if (place === 2)
            return 'Athlete Beta: 2nd athlete to join the Longevity World Cup';
        if (place === 3)
            return 'Athlete Gamma: 3rd athlete to join the Longevity World Cup';
        if (place && place >= 4 && place <= 10)
            return 'Early Bird: among the first 10 athletes to join the Longevity World Cup';
    ...
    if (isSeasonBadgeLabel(label)) {
        ...
        if (place === 1)
            return `${tag} · winner: finished 1st in the ${seasonText}`;
        if (place === 2)
            return `${tag} · 2nd place: finished 2nd in the ${seasonText}`;
        if (place === 3)
            return `${tag} · 3rd place: finished 3rd in the ${seasonText}`;
        if (place && place >= 4 && place <= 10)
            return `${tag} · top 10: finished among the top 10 in the ${seasonText}`;
        if (place && place >= 11 && place <= 20)
            return `${tag} · top 20: finished among the top 20 in the ${seasonText}`;
        return `${tag} · seasonal finish`;
    }
    if (label === 'Podcast')
        return "Podcast: hear this athlete's story in depth";
    if (label === 'Pregnancy')
        return 'Baby on board';
    if (label === 'Host')
        return 'Host: organizer of the Longevity World Cup';
    if (label === 'Perfect application')
        return 'Perfect application: most flawless entry form ever submitted';
...
function computeOrder(b) {
    ...
    const cat = (getCat(b) || '').toLowerCase();
    const place = getPlace(b);
    const isMedalFamily = label === 'Age reduction' ||
        (typeof label === 'string' && label.startsWith('Crowd – '));
    if (isMedalFamily && place && [1, 2, 3].includes(place)) {
        ...
        else if (label === 'Crowd – most guessed')
            micro = 0.20;
        else if (label === 'Crowd – age gap (chrono−crowd)')
            micro = 0.21;
        else if (label === 'Crowd Age – lowest')
            micro = 0.22;
        return base + micro;
    }
    if (label === 'Podcast')
    ...
    if (label === '≥2 submissions')
        return 1.18;
    if (isSeasonBadgeLabel(label) && place)
        return 1.19;
    if (label === 'Pheno Age – lowest')
        return 2.10;
    if (label === 'Pheno Age best improvement')
...
window.setBadges = function (athlete, athleteCell) {
    ...
    const serverBadges = Array.isArray(athlete.Badges) ? athlete.Badges :
    ...
    try {
        ...
        if (rawName) {
            ...
            const parsedGuesses = JSON.parse(localStorage.getItem('gmaAllGuesses') || '{}');
    ...
    const isModalStrip = (badgeContainer.id && badgeContainer.id === 'modalBadgeStrip') ||
