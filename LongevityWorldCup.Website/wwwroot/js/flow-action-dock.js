...
(function () {
    ...
    function updateActionLayoutState(element) {
        ...
        element.querySelectorAll('.flow-action[data-flow-dock-label]').forEach(action => {
            ...
            if (label)
                label.dataset.flowDockLabel = action.dataset.flowDockLabel;
        });
        const backAction = visibleActions.find(isBackAction);
        const primaryAction = visibleActions.find(action => action !== backAction && !action.classList.contains('flow-action--secondary'));
    ...
    function unregisterDisconnectedElements() {
        Array.from(states.entries()).forEach(([element, state]) => {
            if (element.isConnected && state.placeholder.isConnected)
                return;
            if (state.enterFrame)
            ...
            if (state.docked && !state.placeholder.isConnected)
                element.remove();
    ...
    function beginDockEnter(element, state) {
        if (!transitionsReady || reducedMotionMedia.matches)
            return;
    ...
    function shouldDock(element, state) {
        if (!conditionMatches(element))
            return false;
        if (state.docked ? !hasLayoutBox(state.placeholder) : !isVisible(element))
            return false;
        const mode = (element.getAttribute('data-flow-dock') || 'auto').toLowerCase();
        if (mode === 'off')
            return false;
        if (mode === 'mobile' && !mobileMedia.matches)
            return false;
        if (mode === 'always')
            return true;
        if (mode !== 'overflow' && mobileMedia.matches)
            return true;
    ...
    function refresh() {
        ...
        states.forEach((state, element) => {
            ...
            if (docked && !state.docked)
                dockElement(element, state);
            if (!docked && state.docked)
                undockElement(element, state);
            if (!state.docked)
                return;
            updateActionLayoutState(element);
