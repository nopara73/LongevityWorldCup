...
public sealed class EventDataService : IDisposable
{
    ...
    public EventDataService(IWebHostEnvironment env, SlackEventService slackEvents, XEventService xEvents, ThreadsEventService threadsEvents, FacebookEventService facebookEvents, DatabaseManager db, ILogger<EventDataService> log)
    {
        ...
        _db.Run(sqlite =>
        {
            using (var cmd = sqlite.CreateCommand())
            {
                ...
                var addedThreadsProcessed = false;
                cmd.CommandText = "ALTER TABLE Events ADD COLUMN ThreadsProcessed INTEGER NOT NULL DEFAULT 0;";
                try
                {
                    cmd.ExecuteNonQuery();
                    addedThreadsProcessed = true;
                }
                catch
                {
                }

                if (addedThreadsProcessed)
                {
                    cmd.CommandText = "UPDATE Events SET ThreadsProcessed = 1;";
                    cmd.ExecuteNonQuery();
                }

                var addedFacebookProcessed = false;
                cmd.CommandText = "ALTER TABLE Events ADD COLUMN FacebookProcessed INTEGER NOT NULL DEFAULT 0;";
                try
                {
                    cmd.ExecuteNonQuery();
                    addedFacebookProcessed = true;
                }
                catch
                {
                }
    ...
    public void SetAthletesForX(IReadOnlyList<AthleteForX> items)
    {
        ...
        _facebookEvents.SetAthletesForFacebook(items);
        ...
        double defaultRelevance = DefaultRelevanceBadgeAward)
    {
        ...
        if (podcastEvents.Count > 0)
        {
            ...
            {
                var sentX = _xEvents.TrySendEventAsync(EventType.BadgeAward, text).GetAwaiter().GetResult();
                if (sentX)
                    sentXIds.Add(id);

                var sentThreads = _threadsEvents.TrySendEventAsync(EventType.BadgeAward, text).GetAwaiter().GetResult();
                if (sentThreads)
                    sentThreadsIds.Add(id);
            }

            MarkEventsXProcessed(sentXIds);
            MarkEventsThreadsProcessed(sentThreadsIds);
        }
    }
        ...
        double defaultRelevance = 2d)
    {
        ...
        _db.Run(sqlite =>
        {
            ...
            insertCmd.CommandText =
                ...
                "SELECT @id, @type, @text, @occ, @rel, 1, 1, 1, 1 " +
