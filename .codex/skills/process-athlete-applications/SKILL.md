...
- Redact only the applicant's direct private identifiers and unique administrative tokens that are irrelevant to public result verification: exact date of birth, personal phone number or email address, home address, client/patient/medical-record/health/insurance/member numbers, payer identifiers, unique order/accession/specimen/report/account numbers, and barcodes or QR codes encoding those identifiers. Preserve the applicant's name as the ownership signal, and do not redact age, sex, clinical content, or a laboratory value merely because the competition does not use it.
- Preserve laboratory and provider provenance, including laboratory names and logos; clinician, referrer, validator, technician, and signer names; professional or registry IDs; facility and organization identifiers; facility addresses and contact details; doctor phone numbers; and renderer/generator/footer metadata. Do not mistake shared provider information for the applicant's personal contact details.
...
- When `athlete.json` uses a `12/31` DOB privacy placeholder and a proof exposes the exact DOB, censor only the exact DOB and other direct identifiers covered above; preserve the rest of the report and do not use the proof DOB to overwrite the JSON placeholder.
