...
- Challenge timezone edits remain in the open page across views and background updates. Save only the selected change, retain later edits while saving, and protect unfinished work on exit. Keep Save inactive when unchanged and show feedback only for a recoverable failure; saved timezone rules take effect only after an accepted response.
