...
- Proof uploads show an ordered page grid with source filenames, a zoomable reader, and removal undo. Keep preparation progress beside the pages and disable submission while files are processing. Returning to the step retains the attached pages and checklist.
- Long biomarker forms offer an entry overview with the displayed values and units, links back to each field, and a next-unfinished-field action. Partial-update overviews show newly entered values; below-detection selections retain their meaning.
