...
- Profile drafts show their changed fields with direct links back to editing, stay tied to the selected athlete, and clear after successful submission. Resetting all changes offers Undo. Claim a draft is saved only after storage succeeds; protect unsaved work when leaving if storage fails.
